    ...
    loadDictionary(dictData) {
        ...
        for (let entry of dictData) {
            ...
            if (!this.headwords.has(traditional)) {
                const term = new Term(traditional, entry['h'], sense);
...
export class Term {
     ...
     */
    constructor(chinese, headword_id, sense) {
    ...
    getEnglish() {
        ...
        for (let sense of this.senses) {
            ...
            eng = eng.replace(r, ', ');
            console.log(`getEnglish after ${eng}`);
            english += eng + '; ';
        }
...
class WordSense {
     ...
     */
    constructor(chinese, traditional, pinyin, english, grammar) {
